...
class ArchitectureDesigner:
    ...
    def _save_markdown_design(self, design_dir: Path):
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            ...
            if "separation_domains" in self.design:
                for domain_key, domain in self.design["separation_domains"].items():
            ...
            if "interfaces" in self.design:
                for interface_key, interface in self.design["interfaces"].items():
    ...
    def _print_design_summary(self):
        """設計サマリーを表示"""
        print("\n" + "="*60)
        print("Phase4準備: アーキテクチャ設計 - サマリー")
        print("="*60)
        
        # 現在の分析
        if "current_analysis" in self.design:
            analysis = self.design["current_analysis"]
            print(f"現在の構造: {analysis.get('total_lines', 0)}行, {len(analysis.get('classes', []))}クラス, {len(analysis.get('methods', []))}メソッド")
        
        # 分離領域
        if "separation_domains" in self.design:
            domains = self.design["separation_domains"]
            total_estimated_lines = sum(d.get('total_estimated_lines', 0) for d in domains.values())
            print(f"分離後推定: {total_estimated_lines}行 (4領域, {len([c for d in domains.values() for c in d.get('components', [])])}コンポーネント)")
            
            for domain_key, domain in domains.items():
                print(f"  - {domain['name']}: {domain.get('total_estimated_lines', 0)}行 ({domain['risk_level']}リスク)")
        
        # インターフェース
        if "interfaces" in self.design:
            interfaces = self.design["interfaces"]
            print(f"インターフェース: {len(interfaces)}個")
        ...
        if "migration_strategy" in self.design:
            ...
            if "overall_approach" in strategy:
                ...
                print(f"移行期間: {approach.get('duration', 'Unknown')}")
                print(f"移行戦略: {approach.get('strategy', 'Unknown')}")
        
        print("="*60)
        print("設計完了: Phase4実行準備が整いました")
        print("="*60)


def main():
    """メイン実行関数"""
    print("Phase4準備: アーキテクチャ設計スクリプト")
    print("folder_tree.py の分離後アーキテクチャを詳細設計します")
    print()
    
    # 設計実行
    designer = ArchitectureDesigner()
    design = designer.design_complete_architecture()
    
    print("\n✅ アーキテクチャ設計が正常に完了しました")
    print("Phase4実行時にこの設計に基づいて分離を実行します")
    return 0

    ...
    def _print_summary(self):
        ...
        if "tests" in self.results:
            ...
            if "imports" in tests:
                ...
                sum(1 for t in imports.values() if t.get("status") == "SUCCESS")
                len(imports)

            # 初期化テスト
            if "initialization" in tests:
                init = tests["initialization"]
                sum(1 for t in init.values() if t.get("status") == "SUCCESS")
                len(init)

            # 機能テスト
            if "functionality" in tests:
                func = tests["functionality"]
                sum(1 for t in func.values() if t.get("status") == "SUCCESS")
                len(func)

...
from src.core.indexing_worker import IndexingWorker, IndexingStatistics, IndexingProgress
...
class TestIndexingWorkerIntegration:
    ...
    def test_progress_reporting(self):
        ...
        indexing_signals = [s for s in self.progress_signals if "インデックス" in s[0]]
        
        assert len(scanning_signals) > 0, "スキャン進捗が報告されていません"
        assert len(processing_signals) > 0, "処理進捗が報告されていません"
        
        # 進捗率が適切に計算されていることを確認
        for msg, current, total in self.progress_signals:
            if total > 0:

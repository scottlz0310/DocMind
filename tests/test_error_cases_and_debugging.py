...
from src.utils.exceptions import DocumentProcessingError, IndexingError
...
class TestErrorCasesAndDebugging:
    ...
    def test_file_access_permission_errors(self):
        ...
        try:
            ...
            assert len(self.completion_signals) == 1, "権限エラーがあっても処理が完了していません"
            
            folder_path, stats = self.completion_signals[0]
            
            # エラーが記録されていることを確認
            assert len(stats['errors']) > 0 or stats['files_failed'] > 0, \
                "権限エラーが記録されていません"
    ...
    def test_corrupted_file_handling(self):
        ...
        assert len(self.completion_signals) == 1, "破損ファイルエラーがあっても処理が完了していません"
        ...
        assert "corrupted.txt" in error_messages or "破損" in error_messages, \
    ...
    def test_memory_exhaustion_handling(self):
        ...
        assert len(self.completion_signals) == 1, "メモリエラーがあっても処理が完了していません"
    ...
    def test_database_connection_errors(self):
        ...
        assert len(self.error_signals) > 0, "データベースエラーシグナルが発行されていません"
        
        # エラーコンテキストを確認
        error_contexts = [signal[0] for signal in self.error_signals]
        assert any("batch_processing" in context for context in error_contexts), \
            "バッチ処理エラーが記録されていません"
    ...
    def test_file_watcher_initialization_errors(self):
        ...
        self.file_watcher.add_watch_path.side_effect = Exception("ファイル監視初期化エラー")
        ...
        assert len(self.error_signals) > 0, "FileWatcherエラーシグナルが発行されていません"
        
        error_contexts = [signal[0] for signal in self.error_signals]
        assert any("file_watching" in context for context in error_contexts), \
            "ファイル監視エラーが記録されていません"
    ...
    def test_unicode_and_encoding_errors(self):
        ...
        assert len(self.completion_signals) == 1, "エンコーディングエラーがあっても処理が完了していません"
    ...
    def test_concurrent_access_conflicts(self):
        ...
        completed_workers = sum(1 for completions in all_completion_signals if len(completions) > 0)
    ...
    def test_error_logging_and_reporting(self):
        ...
        if log_file.exists():
            ...
            assert "ログテスト用エラー" in log_content or "ERROR" in log_content, \
                "エラーがログファイルに記録されていません"
        
        # エラーシグナルが発行されていることを確認
        assert len(self.error_signals) > 0 or len(self.completion_signals) > 0, \
            "エラー情報が適切に報告されていません"
...
class TestDebuggingFeatures:
    ...
    def test_file_processing_diagnostics(self):
        ...
        for file_info in file_diagnostics:
            ...
            print(f"  {status}: {Path(file_info['file_path']).name} "
    ...
    def test_performance_profiling(self):
        ...
        with patch.object(self.document_processor, 'process_file', side_effect=timed_process_file):
            worker = IndexingWorker(
                str(self.test_folder),
                self.document_processor,
                self.index_manager,
        ...
        # パフォーマンス分析
        if timing_data:
            processing_times = [data['processing_time'] for data in timing_data.values()]
            ...
            # パフォーマンス要件の確認
            assert avg_processing_time < 5.0, f"平均処理時間が長すぎます: {avg_processing_time:.3f}秒"

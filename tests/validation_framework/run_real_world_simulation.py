...
def run_user_scenario_tests(simulator: RealWorldSimulator, scenarios: list) -> dict:
    ...
    if not scenarios or "bulk_processing" in scenarios:
        ...
        try:
            ...
            results["bulk_processing"] = {
                "status": "success",
                "message": "大量データ処理シナリオテスト完了",
            }
        except Exception as e:
...
使用例:
  ...
  python run_real_world_simulation.py --log-level DEBUG --log-file simulation.log
    ...
    parser.add_argument(
        "--enable-memory", action="store_true", default=True, help="メモリ監視を有効化"
    )

    parser.add_argument(
        "--enable-error-injection", action="store_true", help="エラー注入を有効化"
    )

    parser.add_argument(
        "--max-execution-time", type=float, default=600.0, help="最大実行時間（秒）"
    )

    parser.add_argument(
        "--max-memory-usage", type=float, default=3072.0, help="最大メモリ使用量（MB）"
    )
    ...
    parser.add_argument("--output-file", help="結果出力ファイル（JSONフォーマット）")
    ...
    if not args.output_file:
        ...
        args.output_file = (
            f"{args.output_dir}/real_world_simulation_results_{timestamp}.json"
        )
    ...
    try:
        ...
        print("使用パターンテスト実行中...")
        print("=" * 60)
        all_results["usage_patterns"] = run_usage_pattern_tests(
            simulator, args.patterns
        )
        ...
        print("=" * 60)
        all_results["user_scenarios"] = run_user_scenario_tests(
            simulator, args.scenarios
        )

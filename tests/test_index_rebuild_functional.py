...
class TestIndexRebuildFunctional:
    ...
    def temp_folder(self):
        ...
        test_files = [
            ...
            ("functional_test3.md", "# 機能テスト用マークダウン\n\n検索キーワード: マークダウン"),
        ]
    ...
    def test_progress_methods_exist(self, app):
        ...
        try:
            ...
            with patch('src.utils.config.Config.get_data_directory') as mock_data_dir:
                ...
                # 進捗表示メソッドが存在することを確認
                assert hasattr(window, 'show_progress'), "show_progressメソッドが存在しません"
                assert hasattr(window, 'update_progress'), "update_progressメソッドが存在しません"
    ...
    def test_core_components_exist(self, app):
        ...
        try:
            ...
            with patch('src.utils.config.Config.get_data_directory') as mock_data_dir:
                ...
                assert hasattr(window, 'folder_tree_container_container'), "folder_tree_container_containerが存在しません"
    ...
    def test_rebuild_index_with_mocked_dialog(self, app, temp_folder):
        ...
        try:
            ...
            with patch('src.utils.config.Config.get_data_directory') as mock_data_dir:
                ...
                # フォルダを設定
                if hasattr(window, 'folder_tree_container') and hasattr(window.folder_tree_container, 'set_root_folder'):
                    window.folder_tree_container_container.load_folder_structure(temp_folder)
            ...
            with patch('src.utils.config.Config.get_data_directory') as mock_data_dir:
                ...
                with patch.object(QMessageBox, 'question', return_value=QMessageBox.No):
                    ...
                    if hasattr(window, '_rebuild_state'):
                        assert not window._rebuild_state.is_active, "キャンセル後も再構築状態がアクティブです"
...
class TestIndexRebuildPerformance:
    ...
    def test_performance_measurement(self, app, performance_temp_folder):
        ...
        try:
            ...
            with patch('src.utils.config.Config.get_data_directory') as mock_data_dir:
                mock_data_dir.return_value = tempfile.mkdtemp(prefix="docmind_data_perf_test_")
                ...
                # フォルダを設定
                if hasattr(window, 'folder_tree_container') and hasattr(window.folder_tree_container, 'set_root_folder'):
                    window.folder_tree_container_container.load_folder_structure(performance_temp_folder)
                ...
                with patch.object(QMessageBox, 'question', return_value=QMessageBox.Yes):
                    ...
                    while time.time() - start_time < timeout:
                        QApplication.processEvents()
                        if not hasattr(window, '_rebuild_state') or not window._rebuild_state.is_active:
                            break
                        time.sleep(0.1)
                    ...
                    # 最低限の処理速度を確認
                    assert files_per_second > 0.5, f"処理速度が遅すぎます: {files_per_second:.2f} files/sec"

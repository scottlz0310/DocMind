...
class MenuManager(QObject, LoggerMixin):
    ...
    def _create_file_menu(self) -> None:
        ...
        self.actions["open_folder"].triggered.connect(self.main_window.dialog_manager.open_folder_dialog)
    ...
    def _create_search_menu(self) -> None:
        ...
        # 検索実行アクション
        self.actions["search"] = QAction(get_search_icon(), "検索(&S)...", self.main_window)
        self.actions["search"].setShortcut(QKeySequence.Find)
        self.actions["search"].setStatusTip("ドキュメント検索を実行します")
        self.actions["search"].triggered.connect(self.main_window.dialog_manager.show_search_dialog)
        search_menu.addAction(self.actions["search"])

        # インデックス再構築アクション
        self.actions["rebuild_index"] = QAction("インデックス再構築(&R)", self.main_window)
        self.actions["rebuild_index"].setShortcut(QKeySequence("Ctrl+R"))
        self.actions["rebuild_index"].setStatusTip("検索インデックスを再構築します")
        self.actions["rebuild_index"].triggered.connect(self.main_window.index_controller.rebuild_index)
        search_menu.addAction(self.actions["rebuild_index"])

        # インデックスクリアアクション
        self.actions["clear_index"] = QAction("インデックスクリア(&C)", self.main_window)
        self.actions["clear_index"].setShortcut(QKeySequence("Ctrl+Shift+C"))
        self.actions["clear_index"].setStatusTip("検索インデックスをクリアします")
        self.actions["clear_index"].triggered.connect(self.main_window.index_controller.clear_index)
    ...
    def _create_view_menu(self) -> None:
        ...
        # プレビューペイン表示切り替え
        self.actions["toggle_preview"] = QAction("プレビューペイン(&P)", self.main_window)
        ...
        self.actions["toggle_preview"].setStatusTip("プレビューペインの表示を切り替えます")
    ...
    def _create_tools_menu(self) -> None:
        ...
        # 設定アクション
        self.actions["settings"] = QAction(get_settings_icon(), "設定(&S)...", self.main_window)
    ...
    def _create_help_menu(self) -> None:
        ...
        self.actions["about"].triggered.connect(self.main_window.dialog_manager.show_about_dialog)
    ...
    def enable_action(self, action_name: str, enabled: bool = True) -> None:
        ...
        if action:
            ...
            self.logger.debug(f"アクション '{action_name}' を{'有効' if enabled else '無効'}にしました")

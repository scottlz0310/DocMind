...
class LayoutManager(QObject, LoggerMixin):
    ...
    def setup_ui(self) -> None:
        ...
        self.main_window.main_splitter.setCollapsible(0, False)  # 左ペインは折りたたみ不可
        self.main_window.main_splitter.setCollapsible(1, False)  # 中央ペインは折りたたみ不可
        self.main_window.main_splitter.setCollapsible(2, True)   # 右ペインは折りたたみ可能
    ...
    def create_folder_pane(self) -> QWidget:
        ...
        # シグナル接続
        self.main_window.folder_tree_container.folder_selected.connect(self.main_window._on_folder_selected)
        self.main_window.folder_tree_container.folder_indexed.connect(self.main_window._on_folder_indexed)
        self.main_window.folder_tree_container.folder_excluded.connect(self.main_window._on_folder_excluded)
        self.main_window.folder_tree_container.refresh_requested.connect(self.main_window._on_folder_refresh)
    ...
    def create_search_pane(self) -> QWidget:
        ...
        # 検索インターフェースのシグナル接続
        self.main_window.search_interface.search_requested.connect(self.main_window._on_search_requested)
        self.main_window.search_interface.search_cancelled.connect(self.main_window._on_search_cancelled)

        # 検索提案機能の接続
        self.main_window.search_interface.search_input.textChanged.connect(self.main_window._on_search_text_changed)

        # 検索結果ウィジェットのシグナル接続
        self.main_window.search_results_widget.result_selected.connect(self.main_window._on_search_result_selected)
        self.main_window.search_results_widget.preview_requested.connect(self.main_window._on_preview_requested)
        self.main_window.search_results_widget.page_changed.connect(self.main_window._on_page_changed)
        self.main_window.search_results_widget.sort_changed.connect(self.main_window._on_sort_changed)
        self.main_window.search_results_widget.filter_changed.connect(self.main_window._on_filter_changed)
    ...
    def create_preview_pane(self) -> QWidget:
        ...
        # シグナル接続
        self.main_window.preview_widget.zoom_changed.connect(self.main_window._on_preview_zoom_changed)
        self.main_window.preview_widget.format_changed.connect(self.main_window._on_preview_format_changed)
    ...
    def setup_menu_bar(self) -> None:
        ...
        # フォルダを開くアクション
        self.main_window.open_folder_action = QAction("フォルダを開く(&O)...", self.main_window)
        self.main_window.open_folder_action.setShortcut(QKeySequence.Open)
        self.main_window.open_folder_action.setStatusTip("検索対象のフォルダを選択します")
        self.main_window.open_folder_action.triggered.connect(self.main_window._open_folder_dialog)
        ...
        # 検索実行アクション
        self.main_window.search_action = QAction(get_search_icon(), "検索(&S)...", self.main_window)
        self.main_window.search_action.setShortcut(QKeySequence.Find)
        self.main_window.search_action.setStatusTip("ドキュメント検索を実行します")
        self.main_window.search_action.triggered.connect(self.main_window._show_search_dialog)
        search_menu.addAction(self.main_window.search_action)

        # インデックス再構築アクション
        self.main_window.rebuild_index_action = QAction("インデックス再構築(&R)", self.main_window)
        self.main_window.rebuild_index_action.setShortcut(QKeySequence("Ctrl+R"))
        self.main_window.rebuild_index_action.setStatusTip("検索インデックスを再構築します")
        self.main_window.rebuild_index_action.triggered.connect(self.main_window.index_controller.rebuild_index)
        search_menu.addAction(self.main_window.rebuild_index_action)

        # インデックスクリアアクション
        self.main_window.clear_index_action = QAction("インデックスクリア(&C)", self.main_window)
        self.main_window.clear_index_action.setShortcut(QKeySequence("Ctrl+Shift+C"))
        self.main_window.clear_index_action.setStatusTip("検索インデックスをクリアします")
        self.main_window.clear_index_action.triggered.connect(self.main_window.index_controller.clear_index)
        ...
        # プレビューペイン表示切り替え
        self.main_window.toggle_preview_action = QAction("プレビューペイン(&P)", self.main_window)
        ...
        self.main_window.toggle_preview_action.setStatusTip("プレビューペインの表示を切り替えます")
        self.main_window.toggle_preview_action.triggered.connect(self.main_window._toggle_preview_pane)
        ...
        # 設定アクション
        self.main_window.settings_action = QAction(get_settings_icon(), "設定(&S)...", self.main_window)
        ...
        self.main_window.settings_action.triggered.connect(self.main_window._show_settings_dialog)
        ...
        # バージョン情報アクション
        self.main_window.about_action = QAction("DocMindについて(&A)...", self.main_window)
        self.main_window.about_action.setStatusTip("アプリケーションの情報を表示します")
        self.main_window.about_action.triggered.connect(self.main_window._show_about_dialog)
    ...
    def setup_status_bar(self) -> None:
        ...
        self.main_window.status_bar.addPermanentWidget(self.main_window.system_info_label)
    ...
    def setup_shortcuts(self) -> None:
        ...
        # Escキーでプレビューペインをクリア
        self.main_window.clear_preview_shortcut = QShortcut(QKeySequence("Escape"), self.main_window)
        self.main_window.clear_preview_shortcut.activated.connect(self.main_window._clear_preview)

        # F5キーでリフレッシュ
        self.main_window.refresh_shortcut = QShortcut(QKeySequence("F5"), self.main_window)
        self.main_window.refresh_shortcut.activated.connect(self.main_window._refresh_view)

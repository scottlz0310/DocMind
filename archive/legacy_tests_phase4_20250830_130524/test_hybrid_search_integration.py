...
class TestHybridSearchIntegration(unittest.TestCase):
    ...
    def test_semantic_search_integration(self):
        ...
        self.assertTrue(
            found_ml_doc, "セマンティック検索で機械学習関連文書が見つからない"
        )
    ...
    def test_hybrid_search_integration(self):
        ...
        found_data_analysis = any(
            "データ分析" in result.document.content
            or "data_analysis" in result.document.file_path
            for result in results
        )
        self.assertTrue(
            found_data_analysis, "ハイブリッド検索でデータ分析関連文書が見つからない"
        )
    ...
    def test_search_performance(self):
        ...
        query = SearchQuery(
            query_text="機械学習 データ分析", search_type=SearchType.HYBRID, limit=10
        )
        ...
        self.assertLess(
            search_time, 5.0, f"検索時間が5秒を超えました: {search_time:.2f}秒"
        )
    ...
    def test_highlighted_terms_extraction(self):
        """ハイライト用語抽出のテスト"""
        query = SearchQuery(
            query_text="Python データ分析", search_type=SearchType.FULL_TEXT, limit=5
        )
        ...
        for result in results:
            ...
            if result.highlighted_terms:
                ...
                any(any(qt in ft for qt in query_terms) for ft in found_terms)
